...
<html lang="cn">
	<head>
		...
		<meta name="description" content="微商城 微信商城 www.csi0n.com" />
...
<div class="col-sm-12 widget-container-span">
    <div class="widget-box transparent">
                                    ...
                                    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$result): $mod = ($i % 2 );++$i;?><tr>
                                                    ...
                                                    <td>
                                                        <?php if(($result["is_top"]) == "0"): ?><a class="J_ajax_del btn btn-white btn-sm" href="<?php echo U('Admin/Good/setTop',array('id'=>$result['id'],'d'=>1));?>">置顶</a>
                                                            <?php else: ?>
                                                            <a class="J_ajax_del btn btn-white btn-sm" href="<?php echo U('Admin/Good/setTop',array('id'=>$result['id'],'d'=>0));?>">取消置顶</a><?php endif; ?>
                                                        <a href="javascript:void(0);" onclick="reGood(this);" class="btn btn-white btn-sm">修改</a><a class="J_ajax_del btn btn-white btn-sm" href="<?php echo U('Admin/Good/delgood',array('id'=>$result['id']));?>">删除</a></td>
        ...
        function cancelTop(o) {
            ...
            $.post("<?php echo U('Admin/Good/setTop');?>", {
                "id": goodid,
                "d": 0
            }, function(data) {
                alert(data);
        ...
        function reGood(o) {
            var goodid = $(o).parent().prev().prev().prev().prev().prev().html();
            $.post("<?php echo U('Admin/Good/getgoodid');?>", {
                "id": goodid
            }, function(data) {
                var json = eval(data);
